    ...
    <div className="min-h-screen flex flex-col p-4 bg-[#050508] text-white overflow-x-hidden">
      ...
      <main className="flex-grow flex flex-col gap-4">
        ...
        {competition.status === "finished" && (
          <div className="flex-grow flex flex-col items-center justify-center p-6 space-y-8" dir="rtl">
            <div className="bg-[#0a0a12] p-12 rounded-[50px] border-4 border-[#ffd700] text-center shadow-2xl max-w-2xl w-full">
              <Trophy className="w-20 h-20 text-[#ffd700] mx-auto mb-6" />
              <h3 className="text-[#ffd700] font-bold mb-2 uppercase">بطل المسابقة</h3>
              <div className="text-5xl md:text-7xl font-black text-white mb-8">{competition.winnerName}</div>
              {myAnswerData && (
                <div className={cn("p-6 rounded-3xl border mb-8", myAnswerData.isCorrect ? "bg-green-500/10 border-green-500/20 text-green-500" : "bg-red-500/10 border-red-500/20 text-red-500")}>
              ...
              <div className="bg-white/5 p-6 rounded-3xl border border-white/5">
                {competition.prizeImageUrl && <img src={competition.prizeImageUrl} className="w-32 h-32 mx-auto object-contain mb-4" alt="P" />}
                <div className="text-2xl font-bold text-[#ffd700]">{competition.title}</div>
              </div>
            </div>
            {student.uid === competition.winnerId && <WinnerCertificate winnerName={student.name} compTitle={competition.title} date={new Date().toLocaleDateString('ar-SA')} prizeImageUrl={competition.prizeImageUrl} />}
          </div>
        )}
      </main>
